...
'''def checar_perigo(image, animais):
    ...
    for lobo in animais['lobo']:
        x_lobos.append(lobo[0])
        x_lobos.append(lobo[2])
        y_lobos.append(lobo[1])
        y_lobos.append(lobo[3])
        box[0] = min(x_lobos)
        box[1] = min(y_lobos)
        box[2] = max(x_lobos)
        box[3] = max(y_lobos)
    for vaca in animais['vaca']:
        iou =  calcula_iou(vaca, box)
        if iou > 0:
            cv2.putText(image, "Perigo", (vaca[0], vaca[1]), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 2)
        else:
            cv2.putText(image, "Seguro", (vaca[0], vaca[1]), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 0, 0), 2)
    return image'''